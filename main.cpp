// g++ -I"C:\Users\HP-PC\OneDrive\Desktop\rubiks-cube-solver\eigen\eigen-3.4.0" -I"C:\Users\HP-PC\OneDrive\Desktop\rubiks-cube-solver\freeglut\include" -L"C:\Users\HP-PC\OneDrive\Desktop\rubiks-cube-solver\freeglut\lib" main.cpp -lfreeglut -lopengl32 -o main.exe
#include <Eigen/Eigen>
#include <math.h>
#include <time.h>
#include <ctype.h>
...
float button::clickdelay = max(static_cast<double>(10), 1000.0/(CUBE_SIZE*sqrt(CUBE_SIZE)));
...
double transitionSpeed = min(1.0, pow(10.0, floor((CUBE_SIZE-3)/10))/1000.0);
...
__attribute__((force_align_arg_pointer)) void changeState() {
	...
	if (rotationType==1) 
		rubiksCube.front_anticlock();
	else if (rotationType==2) 
		rubiksCube.back_anticlock();
	else if (rotationType==3) 
		rubiksCube.left_anticlock();
	else if (rotationType==4) 
		rubiksCube.right_anticlock();
	else if (rotationType==5) 
		rubiksCube.up_anticlock();
	else if (rotationType==6) 
		rubiksCube.down_anticlock();

	else if (rotationType==7) 
		rubiksCube.front_clock();
	else if (rotationType==8)
		rubiksCube.back_clock();
	else if (rotationType==9)
		rubiksCube.left_clock();
	else if (rotationType==10)
		rubiksCube.right_clock();
	else if (rotationType==11)
		rubiksCube.up_clock();
	else if (rotationType==12)
		rubiksCube.down_clock();
...
__attribute__((force_align_arg_pointer)) double* getRotationMatrix(const Quaterniond& q) {
...
__attribute__((force_align_arg_pointer)) void mouseWheel(int button, int dir, int x, int y){
...
__attribute__((force_align_arg_pointer)) void motion(int x, int y) {
...
void drawCube(float *a, float *b, float *c, float *d, 
	...
	*/
	...
	if (z==0)
		front = rubiksCube.faces[0][y][x];
	if (z==CUBE_SIZE-1)
		back = rubiksCube.faces[1][y][CUBE_SIZE-1-x];

	if (y==0)
		up = rubiksCube.faces[4][CUBE_SIZE-1-z][x];
	if (y==CUBE_SIZE-1)
		down = rubiksCube.faces[5][z][x];

	if (x==0)
		left = rubiksCube.faces[2][y][CUBE_SIZE-1-z];
	if (x==CUBE_SIZE-1)
		right = rubiksCube.faces[3][y][z];
...
__attribute__((force_align_arg_pointer)) void updateRotation() {
...
void displayAbout() {
	...
	printText(-3.8, 3.7, "  RAGHAV ARORA ", 1.2, GLUT_STROKE_MONO_ROMAN, colorList[4], 2);
...
__attribute__((force_align_arg_pointer)) void init() {
